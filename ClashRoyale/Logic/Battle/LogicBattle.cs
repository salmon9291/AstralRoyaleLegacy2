    ...
    public class LogicBattle : List<Player>
    {
        ...
        public LogicBattle(bool isFriendly, int arena)
        {
            ...
            else if (arena == 11)
            {
                Location = Csv.Tables.Get(Csv.Files.Locations)
                           .GetData<Locations>(Csv.Tables.Get(Csv.Files.Arenas)
                                .GetDataWithInstanceId<Arenas>(8 - 1).PvpLocation).GetInstanceId()+1;
            } 
            else
            {
                Location = Csv.Tables.Get(Csv.Files.Locations)
        ...
        /// <param name="isFriendly"></param>
        /// <param name="arena"></param>
        public LogicBattle(bool isFriendly, int arena, bool isTournament)
        {
            IsFriendly = isFriendly;
            IsTournament = true;
            Arena = arena;
...
// Torre 1
            packet.WriteVInt(e); // Asignado a Player
            packet.WriteVInt(13);
            ...
            // Torre 2
            packet.WriteVInt(p); // Asignado a Enemy
            packet.WriteVInt(13);
        ...
        public void EncodeTournament(IByteBuffer packet)
        {
            #region SectorState
            ...
            packet.WriteBoolean(false); // isInOvertime
            packet.WriteBoolean(true); // isTournamentMode
                ...
                foreach (var player in ToArray())
                    if (player.Device.IsConnected)
                    {
                        if (player.Device.SecondsSinceLastCommand > 2)
                        {
                            ...
                            // TODO: Profile Three Crown Wins
                            player.Home.TotalThreeCrownWins += 1;
